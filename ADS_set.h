...
public:
  ADS_set() : table{new Table[N]}, max_sz{N} {}
  ...
  {
    if (Bucket * pos{find_pos2(key)})
      return const_iterator{pos, (int)hash_idx(key), table, max_sz};
    return end();
  }
  ...
  {
    o << "current size: " << sz << ", max size: " << max_sz << std::endl;
    for (int i = 0; i < max_sz; i++)
    {
      o << i << ":";
      if (table[i].head != nullptr)
...
public:
  ...
  void skip()
  {
    while (table[element].head == nullptr && element < max_sz)
    {
      element++;
    }
  }
  ...
  Iterator &operator++()
  {
    if (pos->next != nullptr && pos != nullptr)
    {
      pos = pos->next;
    }
    else
